...
def load(cfg):
    ...
    for arm, _, _ in SERIES:
        ...
        for path in glob.glob(f"{cfg['runs']}/{cfg['pattern'].format(arm=arm)}"):
            ...
            vertices.append((float(path.rsplit("-rate", 1)[1][:-5]),
...
def main() -> None:
    ...
    s = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" '
         ...
         f'<text x="{L}" y="62" font-size="13" fill="{INK2}">Dots: one per request, over its 40-request window. Lines: run medians per offered rate. '
         f'Brackets: ToolTrie-v1 vs no reordering.</text>']
    ...
    s.append(f'<text x="{(L+W-R)/2:.0f}" y="{H-B+50:.0f}" font-size="13" fill="{INK2}" text-anchor="middle">interactivity: responses per second per user (1 / time to first token)</text>')
    s.append(f'<text transform="translate(24,{(T+H-B)/2:.0f}) rotate(-90)" font-size="13" fill="{INK2}" text-anchor="middle">throughput (requests per second)</text>')
    ...
    for rate in cfg["brackets"]:
        ...
        if by is not None:   # vertical leg: throughput at the same interactivity
            ...
            report.append(f"at {rate:g} req/s no-reordering vertex: throughput {dy:+.1f}%")
        if cx is not None:   # horizontal leg: interactivity at the same throughput
            ...
            report.append(f"at {rate:g} req/s no-reordering vertex: interactivity {dx:+.1f}%")
    ...
    for i, (_, label, colour) in enumerate(SERIES):
        y = ly0 + 24 + i * 22
        s.append(f'<line x1="{lx0}" y1="{y-4}" x2="{lx0+22}" y2="{y-4}" stroke="{colour}" stroke-width="2.6"/>')
        s.append(f'<circle cx="{lx0+11}" cy="{y-4}" r="4" fill="{colour}" stroke="{SURFACE}" stroke-width="1.8"/>')
        s.append(f'<text x="{lx0+30}" y="{y}" font-size="12.5" fill="{INK}">{label}</text>')
    for i, line in enumerate(cfg["caption"]):
        s.append(f'<text x="{lx0}" y="{ly0+130+17*i}" font-size="11.5" fill="{INK2}">{line}</text>')

    n = sum(len(c) for c in cloud.values())
    s.append(f'<text x="{L}" y="{H-44}" font-size="11.5" fill="{INK3}">Offered rates {cfg["rates_text"]} req/s, 200 requests each ({n:,} dots). Each dot is the {K} requests that finished nearest to one request: median</text>')
    ...
    s.append(f'<text x="{L}" y="{H-10}" font-size="11.5" fill="{INK3}">median lines by linear interpolation. Source: {cfg["source"]}.</text>')
    s.append("</svg>")
    Path(cfg["out"]).write_text("\n".join(s))
    clipped = sum(1 for c in cloud.values() for x, y in c if not inside(x, y))
